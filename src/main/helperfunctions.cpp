...
QByteArray GetAutoGenCpp(SignalEmiter *rootobj, QString strProjName)
{
	...
	config += QString("// Autogenerated on: %1 \n").arg(QDateTime::currentDateTime().toString("dd-MM-yyyy HH:MM:ss")).toLatin1();
...
void ConfigAllWidgets(QObject *object, QByteArray &config, QByteArray tabs /*= ""*/, QByteArray parentid /*= ""*/)
{
	...
	if (!object->property("Cpp_instantiate").toString().isEmpty())
	{
		...
		for (int i = 1; i < count; i++)
		{
			...
			name = QString::fromLatin1(metaproperty.name()).toLatin1();
			if (name.contains("Cpp_") && name.compare("Cpp_instantiate") != 0 && name.compare("Cpp_declare") != 0 )
			{
				...
				//
				value = object->property(name.toStdString().c_str()).toString().toLatin1();
				if (!value.isEmpty())
				{
					config += "\t" + tabs + value + "\n";
				}
	...
	QObjectList objList = object->children();
	QByteArray parid = object->property("Wt_id").toString().toLatin1();
...
QByteArray ProcessHEADtag(QWebElement *element, QByteArray filepath /*= ""*/)
{
	...
	for (int i = 0; i < objList.count(); i++)
	{
		...
		if(strReadVal.compare("stylesheet", Qt::CaseInsensitive) == 0)
		{
			...
			{
				listStyleSheetPaths.append(strReadVal.toLatin1());
			}
		}
	}
...
QByteArray ProcessDIVtag(QWebElement *element)
{
	...
	{   // is WContainterWidget
		...
		strAttr = element->attribute("id").toLatin1();
		if (!strAttr.isEmpty()) { output += "Wt_id=\"" + strAttr + "\" "; }
		else { output += QString("Wt_id=\"div_%1\" ").arg(counter).toLatin1(); }
	}
	else
	{   // is WText
		htext = element->toInnerXml().toLatin1();
		htext.replace("<","&lt;");
		htext.replace(">","&gt;");
		htext.replace('"',"'");
		htext.replace("\n"," ");
		...
		strAttr = element->attribute("id").toLatin1();
		if (!strAttr.isEmpty()) { output += "Wt_id=\"" + strAttr + "\" "; }
		else { output += QString("Wt_id=\"div_text_%1\" ").arg(counter).toLatin1(); }
	}
	
	// create common attributes
	strAttr = element->attribute("class").toLatin1();
...
QByteArray ProcessAtag(QWebElement *element)
{
	...
	strAttr = element->attribute("id").toLatin1();
	if (!strAttr.isEmpty()) { output += "Wt_id=\"" + strAttr + "\" "; }
	else { output += QString("Wt_id=\"a_%1\" ").arg(counter).toLatin1(); }
	strAttr = element->attribute("class").toLatin1();
	if (!strAttr.isEmpty()) { output += "Wt_styleClass=\"" + strAttr + "\" "; }

	// create specific attributes
	strAttr = element->attribute("href").toLatin1();
	...
	htext = element->toInnerXml().trimmed().toLatin1();
	if (!htext.isEmpty())
	{
		htext.replace("<","&lt;");
		htext.replace(">","&gt;");
		...
		output += ">  <WObject ";
		output += "Wt_className=\"WText\" ";
		output += QString("Wt_id=\"a_text_%1\" ").arg(counter).toLatin1();
		output += "Wt_text=\"" + htext + "\" ";
...
QByteArray ProcessIMGtag(QWebElement *element, QByteArray filepath /*= ""*/)
{
	...
	strAttr = element->attribute("id").toLatin1();
	if (!strAttr.isEmpty()) { output += "Wt_id=\"" + strAttr + "\" "; }
	else { output += QString("Wt_id=\"img_%1\" ").arg(counter).toLatin1(); }
	strAttr = element->attribute("class").toLatin1();
	if (!strAttr.isEmpty()) { output += "Wt_styleClass=\"" + strAttr + "\" "; }

	// create specific attributes
	strAttr = element->attribute("src").toLatin1();
...
QByteArray ProcessSPANtag(QWebElement *element)
{
	...
	strAttr = element->attribute("id").toLatin1();
	if (!strAttr.isEmpty()) { output += "Wt_id=\"" + strAttr + "\" "; }
	else { output += QString("Wt_id=\"span_%1\" ").arg(counter).toLatin1(); }
	strAttr = element->attribute("class").toLatin1();
	if (!strAttr.isEmpty()) { output += "Wt_styleClass=\"" + strAttr + "\" "; }
...
QByteArray ProcessINPUTtag(QWebElement *element)
{
	...
	strAttr = element->attribute("id").toLatin1();
	if (!strAttr.isEmpty()) { output += "Wt_id=\"" + strAttr + "\" "; }
	else { output += QString("Wt_id=\"input_%1\" ").arg(counter).toLatin1(); }
	strAttr = element->attribute("class").toLatin1();
	if (!strAttr.isEmpty()) { output += "Wt_styleClass=\"" + strAttr + "\" "; }

	// create specific attributes
	strAttr = element->attribute("placeholder").toLatin1();
...
QByteArray ProcessBUTTONtag(QWebElement *element)
{
	...
	if(domelem.childNodes().size() > 1)
	{
		...
		strAttr = element->attribute("id").toLatin1();
		if (!strAttr.isEmpty()) { output += "Wt_id=\"" + strAttr + "\" "; }
		else { output += QString("Wt_id=\"button_text_%1\" ").arg(counter).toLatin1(); }
		strAttr = element->attribute("class").toLatin1();
		if (!strAttr.isEmpty()) { output += "Wt_styleClass=\"" + strAttr + "\" "; }
	...
	else
	{
		...
		strAttr = element->attribute("id").toLatin1();
		if (!strAttr.isEmpty()) { output += "Wt_id=\"" + strAttr + "\" "; }
		else { output += QString("Wt_id=\"button_%1\" ").arg(counter).toLatin1(); }
		strAttr = element->attribute("class").toLatin1();
		if (!strAttr.isEmpty()) { output += "Wt_styleClass=\"" + strAttr + "\" "; }
		// create specific attributes
		strAttr = element->attribute("formaction").toLatin1();
		if (!strAttr.isEmpty()) { output += "Wt_link=\"" + strAttr + "\" "; }
		// get text
		for(QDomNode n = domelem.firstChild(); !n.isNull(); n = n.nextSibling())
		{
...
bool CheckValidProjPath(const QString &strProjPath)
{
	...
	QDirIterator it(m_Path.absolutePath().toLatin1(), QStringList() << "*.wui", QDir::Files, QDirIterator::NoIteratorFlags);
	if (!it.hasNext())
	{
		QMessageBox msgBox;
...
void CopyAllFilesInPath(QString strSourcePath, QString strTargetPath, QStringList strListTypes)
{
	...
	QDirIterator it(m_Path.absolutePath().toLatin1(), strListTypes, QDir::Files, QDirIterator::Subdirectories);
...
void HelpUpdateAllProperties(QObject *rootobj)
{
	if (rootobj)
	{
		// load properties to wtqtobjects
